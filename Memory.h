...
namespace ExMemory
{
	...
	class GTA5 : public Memory
	{
		...
		template<typename T> T GG(int index)
		{
			return r<T>(GA(index));
		}
		template<typename T> void SG(int index, T value)
		{
			w<T>(GA(index), {}, value);
		}
		int64_t LA(std::string name, int index);
		template<typename T> T GL(std::string name, int index)
		{
			return r<T>(LA(name, index));
		}
		template<typename T> void SL(std::string name, int index, T value)
		{
			w<T>(LA(name, index), {}, value);
		}
