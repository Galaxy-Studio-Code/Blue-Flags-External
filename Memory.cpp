...
namespace ExMemory
{
    ...
    int64_t GTA5::LA(std::string name, int index)
    {
        for (int i=0; i < 54; i++)
        {
            ...
            if (str == name)
            {
                return r<int64_t>(g_pointers->LocalScriptsPTR, { i * 0x8, 0xB0 }) + 8 * index;
            }
        }
    }
    ...
    Vector3 GTA5::get_blip_pos(int64_t blip)
    {
        return  r<Vector3>(blip + 0x10);
    }

    int64_t GTA5::get_local_ped()
    {
        return r<int64_t>(g_pointers->WorldPTR, { 0x8 });
    }

    int64_t GTA5::ped_get_current_vehicle(int64_t ped)
    {
        return r<int64_t>(ped + 0xD30);
    }

    bool GTA5::ped_is_in_vehicle(int64_t ped)
    {
        return r<byte>(ped + 0xE52) == 0 ? false : true;
    }
